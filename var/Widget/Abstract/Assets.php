 ...
 * @package Widget
 */
class Widget_Abstract_Assets extends Widget_Abstract
{
    /**
     * 将tags取出
     *
    ...
    {
        $select = $this->db->select(array('COUNT(table.assets.cid)' => 'num'))->from('table.assets')
        ->where("table.assets.{$column} > {$offset}")
        ->where("table.assets.type = ?", $type);

        if (!empty($status)) {
            $select->where("table.assets.status = ?", $status);
        }

        if ($authorId > 0) {
            $select->where('table.assets.authorId = ?', $authorId);
        }
    ...
    {
        return $this->db->select('table.assets.cid', 'table.assets.title', 'table.assets.slug', 'table.assets.created', 'table.assets.authorId',
        'table.assets.modified', 'table.assets.type', 'table.assets.status', 'table.assets.text', 'table.assets.commentsNum', 'table.assets.order',
        'table.assets.template', 'table.assets.password', 'table.assets.allowComment', 'table.assets.allowPing', 'table.assets.allowFeed',
        'table.assets.parent')->from('table.assets');
    }
    ...
    public function insert(array $content)
    {
        ...
        /** 首先插入部分数据 */
        $insertId = $this->db->query($this->db->insert('table.assets')->rows($insertStruct));
    ...
    public function update(array $content, Typecho_Db_Query $condition)
    {
        ...
        $updateRows = $this->db->query($condition->update('table.assets')->rows($updateStruct));
    ...
    {
        return $this->db->query($condition->delete('table.assets'));
    }
    ...
    public function applySlug($slug, $cid)
    {
        ...
        $draft = $this->db->fetchObject($this->db->select('type', 'parent')
            ->from('table.assets')->where('cid = ?', $cid));
        ...
        ->from('table.assets')->where('slug = ? AND cid <> ?', $result, $cid))->num > 0) {
            $result = $slug . '-' . $count;
            $count ++;
        }

        $this->db->query($this->db->update('table.assets')->rows(array('slug' => $result))
        ->where('cid = ?', $cid));
    ...
    public function size(Typecho_Db_Query $condition)
    {
        return $this->db->fetchObject($condition
            ->select(array('COUNT(DISTINCT table.assets.cid)' => 'num'))
            ->from('table.assets')
